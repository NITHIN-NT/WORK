...
export function Topbar({ onMenuClick }: TopbarProps) {
  ...
  return (
    <header 
      className="fixed top-0 right-0 left-0 md:left-64 z-40 bg-white/80 backdrop-blur-xl border-b border-border h-16 transition-all"
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="flex items-center gap-3 pl-2 sm:pl-4 border-l border-border/50 ml-1 sm:ml-2">
            <div className="text-right hidden sm:block leading-tight pr-2">
              <p className="text-xs font-black text-foreground leading-none">{user?.displayName || "Project Manager"}</p>
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Admin Account</p>
            </div>
            ...
            <Button 
              ...
              size="icon" 
              onClick={signOut}
              className="text-zinc-400 hover:text-rose-500 hover:bg-rose-500/5 transition-all h-8 w-8"
            >
